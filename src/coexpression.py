"""
src/coexpression.py — Reusable Co-expression Analysis Logic
"""
...
class Config:
    ...
    def from_yaml(cls, path: Path) -> "Config":
        ...
        # Check for nested 'coexpression' block
        if 'coexpression' in data and isinstance(data['coexpression'], dict):
            data = data['coexpression']
            ...
            try:
                ...
                if "filename" in df.columns and "condition" in df.columns:
                    ...
                    for _, row in df.iterrows():
                        ...
                        # Also map animal ID (first token of key) if possible
                        parts = stem.split("_")
                        if parts:
                            animal = parts[0]
